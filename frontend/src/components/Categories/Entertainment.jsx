...
const Entertainment = () => {
  ...
  const fetchArticles = (language) => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?category=entertainment&language=${language}&pageSize=80&apiKey=${apiKey}`;
  ...
  const fetchGeminiResponse = () => {
    fetch("http://localhost:5000/entertainment-news-summary")
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
  ...
  return (
    <div className='flex flex-col min-h-screen bg-gray-50'>
      <header className='bg-white text-gray-800 p-6 shadow-md'>
        <h1 className='text-4xl font-bold text-center'>Entertainment News</h1>
      </header>
      ...
      <footer className='bg-white text-gray-700 text-center p-4 border-t border-gray-200'>
        <p>&copy; {new Date().getFullYear()} Entertainment News. All rights reserved.</p>
      </footer>
    </div>
  );
};
