...
const Headlines = () => {
  ...
  const fetchArticles = (language) => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?language=${language}&page=1&pageSize=80&apiKey=${apiKey}`;
  ...
  const fetchGeminiResponse = () => {
    fetch("http://localhost:5000/topheadlines-news-summary")
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
  ...
  return (
    <div className='flex flex-col min-h-screen bg-gray-50'>
      <header className='bg-blue-800 text-white p-6 shadow-md'>
        <h1 className='text-3xl font-bold text-center'>Headlines News</h1>
      </header>
      <main className='flex-1 p-6'>
        ...
        <div className='mb-6'>
          <Link to='/' className='inline-flex items-center text-blue-600 hover:text-blue-800 text-sm font-semibold'>
            &larr; Back
          </Link>
        </div>
          ...
          <ul className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
            {filteredArticles.map((article, index) => (
              <li key={index} className='bg-white shadow-md rounded-lg border border-gray-200 p-4 transition-transform transform hover:scale-105'>
                <h3 className='text-md font-semibold mb-2'><b>Source:</b> {article.source.name}</h3>
                <h4 className='text-sm mb-2'><b>Author:</b> {article.author || 'Unknown'}</h4>
                <h4 className='text-sm mb-2'><b>Title:</b> {truncateText(article.title, 50)}</h4>
                {article.urlToImage && (
                  <img src={article.urlToImage} alt={article.source.name} className='w-full h-40 object-cover rounded-md mb-4' />
                )}
      ...
      <footer className='bg-blue-800 text-white text-center p-4'>
        <p>&copy; {new Date().getFullYear()} Headlines News. All rights reserved.</p>
      </footer>
    </div>
  );
};
