...
const Technology = () => {
  ...
  const fetchArticles = (language) => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?category=technology&language=${language}&page=1&pageSize=80&apiKey=${apiKey}`;
  ...
  const fetchGeminiResponse = () => {
    fetch("http://localhost:5000/technology-news-summary")
      .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
  ...
  return (
    <div className='flex flex-col min-h-screen bg-gray-100'>
      <header className='bg-blue-900 text-white p-6 shadow-md'>
        <h1 className='text-3xl font-bold text-center'>Technology News</h1>
      </header>
          ...
          <ul className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
            {filteredArticles.map((article, index) => (
              <li key={index} className='bg-white shadow-lg rounded-lg border border-gray-300 p-4 transition-transform transform hover:scale-105'>
                ...
                <a href={article.url} target="_blank" rel="noopener noreferrer" className='text-blue-600 hover:text-blue-800'>Read more</a>
              </li>
            ))}
          </ul>
        </section>
      ...
      <footer className='bg-blue-900 text-white text-center p-4'>
        <p>&copy; {new Date().getFullYear()} Technology News. All rights reserved.</p>
      </footer>
    </div>
  );
};
